...
class GoodwishEdgeImport {
    ...
    public function import_options($file){
        $options = $this->file_options($file);
        update_option( 'edgt_options_goodwill', $options);
    ...
    function edgt_admin_import() {
        if (edgt_core_theme_installed()) {
            ...
            $this->pagehook = add_submenu_page(
                ...
                array($goodwish_edge_Framework->getSkin(), 'renderImport')
            );

            add_action('admin_print_scripts-'.$this->pagehook, 'goodwish_edge_enqueue_admin_scripts');
            add_action('admin_print_styles-'.$this->pagehook, 'goodwish_edge_enqueue_admin_styles');
	...
	function edgtf_update_options_after_import( $folder ) {
		...
		foreach ( $demo_urls as $demo_url ) {
			$global_options    = get_option( 'edgt_options_goodwill' );
			$new_global_values = str_replace( $demo_url, $url, $global_options );

			update_option( 'edgt_options_goodwill', $new_global_values );
		}
	}
...
function edgt_init_import_object(){
    global $goodwish_edge_import_object;
    $goodwish_edge_import_object = new GoodwishEdgeImport();
}

add_action('init', 'edgt_init_import_object');

if(!function_exists('goodwish_edge_dataImport')){
    function goodwish_edge_dataImport(){
        global $goodwish_edge_import_object;

        if ($_POST['import_attachments'] == 1)
            $goodwish_edge_import_object->attachments = true;
        else
            $goodwish_edge_import_object->attachments = false;

        $folder = "goodwish/";
        if (!empty($_POST['example']))
            $folder = $_POST['example']."/";

        $goodwish_edge_import_object->import_content($folder.$_POST['xml']);

        die();
    }

    add_action('wp_ajax_edgt_dataImport', 'goodwish_edge_dataImport');
}

if(!function_exists('goodwish_edge_widgetsImport')){
    function goodwish_edge_widgetsImport(){
        global $goodwish_edge_import_object;
        ...
        $goodwish_edge_import_object->import_widgets($folder.'widgets.txt',$folder.'custom_sidebars.txt');

        die();
    }

    add_action('wp_ajax_edgt_widgetsImport', 'goodwish_edge_widgetsImport');
}

if(!function_exists('goodwish_edge_optionsImport')){
    function goodwish_edge_optionsImport(){
        global $goodwish_edge_import_object;
        ...
        $goodwish_edge_import_object->import_options($folder.'options.txt');

        die();
    }

    add_action('wp_ajax_edgt_optionsImport', 'goodwish_edge_optionsImport');
}

if(!function_exists('goodwish_edge_otherImport')){
    function goodwish_edge_otherImport(){
        global $goodwish_edge_import_object;
        ...
        $goodwish_edge_import_object->import_options($folder.'options.txt');
        $goodwish_edge_import_object->import_widgets($folder.'widgets.txt',$folder.'custom_sidebars.txt');
        $goodwish_edge_import_object->import_menus($folder.'menus.txt');
        $goodwish_edge_import_object->import_settings_pages($folder.'settingpages.txt');

		$goodwish_edge_import_object->edgtf_update_meta_fields_after_import( $folder );
		$goodwish_edge_import_object->edgtf_update_options_after_import( $folder );

        die();
    }

    add_action('wp_ajax_edgt_otherImport', 'goodwish_edge_otherImport');
}